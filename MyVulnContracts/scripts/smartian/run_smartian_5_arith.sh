...
TAG="${1:?usage: run_smartian_5_arith.sh <tag> <bin> <abi>}"
BIN="${2:?missing .bin}"
ABI="${3:?missing .abi}"
...
for i in 1 2 3 4 5; do
  ...
  for t in $(seq 1 600); do
    # For arithmetic errors, Smartian typically emits "IB_" (Integer Bug) in findings/artifacts
    if grep -q "IB_" "$OUT/stdout.log"; then
      FOUND=1
      echo "[+] Integer Bug (IB) found — stopping run $i"
      break
    fi
  ...
  if [ "$FOUND" -eq 1 ]; then
    TLINE="$(grep -m 1 "IB_" "$OUT/stdout.log" || true)"
