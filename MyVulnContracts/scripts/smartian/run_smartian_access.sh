...
TAG="${1:?usage: run_smartian_5_access.sh <tag> <bin> <abi>}"
BIN="${2:?missing .bin}"
ABI="${3:?missing .abi}"
...
for i in 1 2 3 4 5; do
  ...
  for t in $(seq 1 600); do
    # Access control / TxOrigin findings appear as TO_ artifacts (e.g., id-00000-TO_00000)
    if grep -q "TO_" "$OUT/stdout.log"; then
      FOUND=1
      echo "[+] TxOrigin/Access-Control issue (TO_) found — stopping run $i"
      break
    fi
  ...
  if [ "$FOUND" -eq 1 ]; then
    TLINE="$(grep -m 1 "TO_" "$OUT/stdout.log" || true)"
